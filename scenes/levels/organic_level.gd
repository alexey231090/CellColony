...
func _setup_level_background(level_data: Dictionary) -> void:
	if not bg_rect:
		return
	if _bg_shader_material == null and bg_rect.material != null:
		_bg_shader_material = bg_rect.material
	var chapter: int = int(level_data.get("chapter", 1))
	...
	if level_manager != null and "current_level" in level_manager:
		current_level_num = int(level_manager.current_level)
	elif "id" in level_data:
		current_level_num = int(level_data.id)
	...
	match difficulty:
		"easy":
			return {
				...
				"virus_only_when_outnumbered": false,
		...
		"hard":
			return {
				...
				"virus_only_when_outnumbered": false,
				"ai_virus_energy_cost": 35.0,
				"virus_energy_reserve": 20.0,
				"virus_start_cd": 0.0,
				"virus_min_cd": 20.0,
				"virus_max_shot_range": 3500.0,
				"rapid_fire_hp_target_threshold": 0.55,
				"speed_boost_distance_threshold": 900.0,
			}
		_: # medium (по умолчанию) — умеренный темп перков и защитный вирус только в меньшинстве
			return {
				"decision_interval": 3.5,
				"perk_delay_mult": 2.5,
				"perk_use_interval": 15.0,
				...
				"shield_min_cd": 40.0,
				"virus_min_enemy_count": 3,
				"virus_only_when_outnumbered": true,
				"ai_virus_energy_cost": 50.0,
				"virus_energy_reserve": 25.0,
				"virus_start_cd": 60.0,
				"virus_min_cd": 60.0,
				"virus_max_shot_range": 3500.0,
				"rapid_fire_hp_target_threshold": 0.4,
				"rapid_fire_min_cd": 45.0,
				"speed_min_cd": 40.0,
