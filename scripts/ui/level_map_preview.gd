...
func _draw() -> void:
	...
	var scale_factor_x: float = avail_w / total_world_w
	var scale_factor_y: float = avail_h / total_world_h
	var center_offset: Vector2 = rect_size * 0.5

	# 1. Построение контура арены на всю карточку
	var segments: int = 32
	...
	for i in range(segments):
		...
		var screen_pt: Vector2 = center_offset + Vector2(base.x * scale_factor_x, base.y * scale_factor_y)
	...
	if arena_pts.size() >= 3:
		# Фон арены
		draw_colored_polygon(arena_pts, Color(0.04, 0.08, 0.12, 0.92))
		
		# Обводка арены с учетом палитры главы: мягкое свечение + четкий контур
		var arena_loop := arena_pts.duplicate()
		arena_loop.append(arena_loop[0])
		draw_polyline(arena_loop, ch_colors.highlight, 3.0, true)
		draw_polyline(arena_loop, ch_colors.border, 2.0, true)
	...
	if bool(level_data.get("has_islands", false)):
		...
		for i in range(island_count):
			...
			for s_idx in range(isegs):
				...
				var screen_p: Vector2 = center_offset + Vector2(world_p.x * scale_factor_x, world_p.y * scale_factor_y)
				island_pts.append(screen_p)

			if island_pts.size() >= 3:
				draw_colored_polygon(island_pts, Color(0.02, 0.05, 0.08, 0.95))
				var island_loop := island_pts.duplicate()
				island_loop.append(island_loop[0])
				draw_polyline(island_loop, ch_colors.island_border, 1.8, true)

	# 3. Отрисовка нейтральных клеток (рассеянные точки с контрастным ореолом)
	var neutral_count: int = mini(24, int(level_data.get("num_neutrals", 40)))
	var neutral_seed: int = int(level_data.get("seed", 42)) + 555
	var rng := RandomNumberGenerator.new()
	rng.seed = neutral_seed

	for n in range(neutral_count):
		var nx: float = rng.randf_range(-hx * 0.72, hx * 0.72)
		var ny: float = rng.randf_range(-hy * 0.72, hy * 0.72)
		var n_screen: Vector2 = center_offset + Vector2(nx * scale_factor_x, ny * scale_factor_y)
		# Мягкий ореол и четкое ядро нейтрала
		draw_circle(n_screen, 3.2, Color(COLOR_NEUTRAL.r, COLOR_NEUTRAL.g, COLOR_NEUTRAL.b, 0.20))
		draw_circle(n_screen, 2.0, COLOR_NEUTRAL)

	# 4. Спавны врагов (крупные, с био-свечением и бликом)
	...
	for i in range(1, num_enemies + 1):
		...
		var enemy_pos: Vector2 = center_offset + Vector2(raw_pos.x * scale_factor_x, raw_pos.y * scale_factor_y)
		...
		# Внешнее био-свечение врага
		draw_circle(enemy_pos, 7.0, enemy_color * Color(1, 1, 1, 0.25))
		# Ореол врага
		draw_circle(enemy_pos, 5.0, enemy_color * Color(1, 1, 1, 0.50))
		# Ядро врага
		draw_circle(enemy_pos, 3.8, enemy_color)
		# Блик в центре
		draw_circle(enemy_pos, 1.6, Color(1.0, 1.0, 1.0, 0.95))

	# 5. Спавн игрока (голубая точка с ярким двойным кольцом / ореолом)
	...
	var player_pos: Vector2 = center_offset + Vector2(player_raw_pos.x * scale_factor_x, player_raw_pos.y * scale_factor_y)

	# Внешнее био-свечение спавна игрока
	draw_circle(player_pos, 8.5, COLOR_PLAYER * Color(1, 1, 1, 0.22))
	# Ореол спавна игрока
	draw_arc(player_pos, 7.0, 0.0, TAU, 18, Color(COLOR_PLAYER.r, COLOR_PLAYER.g, COLOR_PLAYER.b, 0.85), 1.6, true)
	# Ядро спавна игрока
	draw_circle(player_pos, 4.2, COLOR_PLAYER)
	# Блик в центре
	draw_circle(player_pos, 1.8, Color(1.0, 1.0, 1.0, 0.98))
